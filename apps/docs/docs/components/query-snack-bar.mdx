...
```tsx
<QuerySnackBar open={open} isSuccess={isSuccess} isError={isError} message={message} autoHideDuration={5000} />
```
...
`QuerySnackBar` uses MUI's `Snackbar`, `Alert`, `IconButton`, `Slide`, and the `Close` icon. Ensure your project has `@mui/material` and `@mui/icons-material` installed (see [Installation](/docs/getting-started/installation)).
