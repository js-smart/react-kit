...
**Next Link** is a link component that uses TanStack Router's [Link](https://tanstack.com/router/latest/docs/framework/react/api/router/linkComponent) with MUI's [Link](https://mui.com/material-ui/react-link/) styling. It renders as an in-app navigation link with hover underline and optional `linkText` or `children`.
...
```tsx
<NextLink href="/help" linkText="Help" target="_self" ariaLabel="Open help page" />
```
...
`NextLink` uses MUI's `Link` and TanStack Router's `Link`. Ensure your project has `@mui/material` and `@tanstack/react-router` installed (see [Installation](/docs/getting-started/installation)).
