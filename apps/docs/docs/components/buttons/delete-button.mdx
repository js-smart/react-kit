...
```tsx
<DeleteButton label="Remove" loading={isDeleting} onClick={() => handleRemove()} />
```
...
```tsx
<DeleteButton label="Delete" loading={isDeleting} loadingLabel="Deleting..." onClick={() => handleDelete()} />
```
...
```tsx
<DeleteButton loading={false} variant="outlined" color="error" onClick={() => handleDelete()} />
```
