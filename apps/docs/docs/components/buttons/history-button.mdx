...
```tsx
<HistoryButton name="History" variant="outlined" color="secondary" onClick={() => navigate('/history')} />
```

## Custom start icon

```tsx
<HistoryButton name="Activity" startIcon={<TimelineIcon />} onClick={() => openActivity()} />
```
